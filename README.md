...
*Every opponent above is written by us, and that is the objection a control engineer makes: nobody would write an `if`, they would pose a **receding-horizon model predictive controller** and solve it. So we did. It re-solves a linear program over the next 6 hours at every hour, commits only the first hour, and minimises the same max-min objective the agent's reward encodes - the worst-served district, through an epigraph variable. It reads exactly what the agent reads and nothing more. Regenerated by `python -m wiiga.mpc`.*

| operator | dry hours / day | worst district | local currency / day | L diesel / day |
|---|---:|---:|---:|---:|
| model predictive control | 0,871 | 0,397 | 140 372 | 11,7 |
| hand-written rulebook | 0,332 | 0,310 | 321 557 | 62,8 |
| **WIIGA agent** | 0,156 | 0,101 | 268 745 | 39,9 |

**The agent serves better by 82 %, and the controller runs the station for 48 % less money.** Both halves are the result, and the second one is not a detail: a linear program minimising cost subject to expected service is very good at the cost half.

**Why it loses the half that matters** is the same fact that defeats a threshold, measured two sections above: outage risk is **bimodal**. A deterministic controller plans against the *expectation* of grid availability - it acts as though half a grid were available on a risky evening. Half a grid does not exist. The evening arrives, the grid is either there or gone, and the plan built on the average was built on a state of the world that never occurs. The agent is not smarter about optimisation; it never had to average a bimodal variable in the first place.

**What this comparison does not settle.** Our MPC does not warn the city and does not hand the hour back, because no optimal-control formulation carries a credibility channel - that is the gap this project fills, and the ablation says speech alone is worth 31 % of the dry hours. A scenario-based stochastic MPC would also close part of the remaining gap, and it is a different piece of work. This is a certainty-equivalent controller with a terminal reserve, which is the standard one, and it is named as such.

...
| We compared against the method a control engineer would actually pose, not only against rules we wrote | a **receding-horizon MPC** solving a linear program every hour on the same information and the same max-min objective: the agent **serves 82 % better**, and the controller **runs the station for 48 % less** - both halves published | `python -m wiiga.mpc` |
